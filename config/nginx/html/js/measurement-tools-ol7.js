 ...
 * Адаптирован для OpenLayers 7.5.1
...
class MeasurementTools {
    ...
    _addInteraction() {
        ...
        // В OpenLayers 7 используем addEventListener вместо on
        this.currentInteraction.addEventListener('drawstart', this._onDrawStart.bind(this));
        this.currentInteraction.addEventListener('drawend', this._onDrawEnd.bind(this));
    }
    ...
    _onDrawStart(evt) {
        ...
        // В OpenLayers 7 используем addEventListener вместо on
        geometry.addEventListener('change', this._onGeometryChange.bind(this));
    }
    ...
    _getLineLength(line) {
        ...
        if (coordinates.length > 1) {
            for (let i = 0, ii = coordinates.length - 1; i < ii; ++i) {
                // В OpenLayers 7 изменился API для трансформации координат
    ...
    _getPolygonArea(polygon) {
        ...
        // В OpenLayers 7 клонирование и трансформация работают иначе
        // Используем clone().transform() вместо polygon.clone().transform()
        const geom = polygon.clone();
        geom.transform(sourceProj, 'EPSG:4326');
    ...
    _pointerMoveHandler(evt) {
        ...
        if (this.sketch) {
            ...
            if (geom instanceof ol.geom.Polygon) {
                ...
                // В OpenLayers 7 нет метода getInteriorPoint, используем getCenter или getCentroid
                try {
                    this.measureTooltip.setPosition(geom.getInteriorPoint().getCoordinates());
                } catch (e) {
                    // Запасной вариант, если getInteriorPoint недоступен
        ...
        if (this.helpTooltipElement) {
            ...
            // Проверяем и добавляем класс hidden, если он отсутствует
