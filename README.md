...
5. **scripts/check-health.ps1** - Скрипт проверки здоровья системы
   - Отображает статус всех компонентов
   - Проверяет доступность сервисов
   - Может автоматически исправлять проблемы

6. **scripts/collect-container-logs.ps1** - Скрипт для сбора и анализа логов контейнеров
...
7. **scripts/check-geoserver-redirect.ps1** - Скрипт для диагностики проблем с редиректами GeoServer
...
8. **scripts/initialize-git-repo.ps1** - Скрипт для инициализации Git репозитория
...
9. **scripts/monitor-geoserver-health.ps1** - Скрипт мониторинга здоровья GeoServer
...
10. **scripts/apply-config-changes.ps1** - Скрипт для применения изменений конфигурации
...
11. **scripts/test-environment.ps1** - Скрипт для тестирования окружения
